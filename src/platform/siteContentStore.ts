 ...
 * config is used unchanged â€” so the site never breaks.
 */
import { defineStore } from 'pinia'
import { ref, computed } from 'vue'
import { siteConfig as buildTimeConfig, type KeystoneSiteConfig } from '../config/site.config'
...
export const useSiteContentStore = defineStore('siteContent', () => {
  const config = ref<KeystoneSiteConfig>(buildTimeConfig)
  ...
  async function hydrate() {
    ...
    try {
      ...
      config.value = deepMerge(buildTimeConfig, res.content) as KeystoneSiteConfig
      hydrated.value = true
    } catch (e) {
      error.value = e instanceof Error ? e.message : String(e)
      // swallow â€” fall back to build-time config
    } finally {
